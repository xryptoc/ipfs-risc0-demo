...
pub async fn select_from_ipfs_generate_guest_input(hash: &str, start: u64, end: u64) -> IpfsProof {
    if hash.chars().next().unwrap() != 'b' {
        panic!("must be base32");
    }
...
#[async_recursion]
pub async fn depth_first_search(hash: &Vec<u8>, current_data_position: u64, start: u64, end: u64, history: Vec<messages::PbNode>, raw_history:Vec<Vec<u8>>) -> (Vec<u8>, u64, Vec<SingleDataEntry>) {
    ...
    let base32_hash = format!("b{}", base32::encode(RFC4648 { padding: false }, &hash.clone()));
    println!("Executing {} {} ", &base32_hash, current_data_position);
    let res = get_block_bytes(&base32_hash).await;
